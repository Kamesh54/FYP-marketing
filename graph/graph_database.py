...
class GraphDatabaseClient:
    ...
    def _initialize_driver(self) -> None:
        ...
        try:
            ...
            # Neo4j 6.1.0 uses simplified configuration
            # Only pass encryption settings for non-secure URI schemes
            driver_kwargs = {
                "auth": (NEO4J_USER, NEO4J_PASSWORD),
            }
            
            # Add encryption parameters only for insecure schemes
            # Secure schemes like neo4j+s:// and bolt+s:// handle encryption automatically
    ...
    def create_node(
        ...
        # Determine unique key property and its name for merge
        unique_key_prop = None
        unique_key_value = None
        if label == "User":
            unique_key_prop = "user_id"
            unique_key_value = properties.get("user_id")
        elif label == "Brand":
            unique_key_prop = "brand_id"
            unique_key_value = properties.get("brand_id")
        elif label == "Campaign":
            unique_key_prop = "campaign_id"
            unique_key_value = properties.get("campaign_id")
        elif label == "Content":
            unique_key_prop = "content_id"
            unique_key_value = properties.get("content_id")
        elif label == "Keyword":
            unique_key_prop = "keyword_id"
            unique_key_value = properties.get("keyword_id") or properties.get("term")
        elif label == "Competitor":
            unique_key_prop = "competitor_id"
            unique_key_value = properties.get("competitor_id") or properties.get("domain")
        elif label == "Metric":
            unique_key_prop = "metric_id"
            unique_key_value = properties.get("metric_id")
        
        if merge and unique_key_value is not None and unique_key_prop:
            # Parameterized MERGE — safe, type-correct, no string interpolation of values
            cypher = f"""
            ...
            RETURN n
            """
            # $unique_key and $props are both top-level parameters
            props = {k: v for k, v in properties.items()}
        else:
            # Fallback: CREATE with all properties as parameters
            props_str = ", ".join([f"{k}: ${k}" for k in properties.keys()])
            cypher = f"CREATE (n:{label} {{{props_str}}}) RETURN n"
            props = properties
        
        try:
            if merge and unique_key_value is not None:
                result = self.execute_write(cypher, {"unique_key": unique_key_value, "props": props})
            else:
    ...
    def create_relationship(
        ...
        """
        params: Dict[str, Any] = {
            "id1": str(node1_id) if node1_id is not None else None,
            "id2": str(node2_id) if node2_id is not None else None,
        }

        if params["id1"] is None or params["id2"] is None:
            logger.warning(f"Skipping relationship {relationship_type}: one or both node IDs are None")
            return False

        # MERGE on type only, then SET properties — avoids mutable-prop MERGE bug
        set_clause = ""
        if properties:
            set_clause = "SET r += $rprops"
            params["rprops"] = properties
        ...
        try:
            with self.get_session() as session:
                ...
                # Consume and check rows returned (MERGE on rel may return 1 row even if 0 created)
                records = list(result)
                summary = result.consume()
                created = summary.counters.relationships_created
                logger.debug(f"Relationship {relationship_type}: {created} created, {len(records)} matched")
